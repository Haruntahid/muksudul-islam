...
import source from "@/data/portfolioSource.json";

export type PortfolioSource = typeof source;
...
export function extractGithubUsername(github: string): string {
  return github
    .replace(/^https?:\/\//, "")
    .replace(/^www\./, "")
    .replace(/^github\.com\//, "")
...
export function getSkillCategoryStyle(name: string) {
  const entry = Object.values(CATEGORY_META).find((c) => c.name === name);
  return entry ?? { name, color: "text-primary", bg: "bg-primary/10 border-primary/20", icons: [] as string[] };
}
...
export function transformPortfolioSource(data: PortfolioSource = source) {
  ...
  return {
    ...
    socials: [
      {
        platform: "GitHub",
        url: normalizeUrl(profile.contact.github),
        icon: "github",
      ...
      {
        platform: "LinkedIn",
        url: normalizeUrl(profile.contact.linkedin),
        icon: "linkedin",
    ...
    settings: {
      ...
      footer: {
        tagline: "Built with Passion for Quality Engineering.",
...
export function toPortfolioFallback(): PortfolioData {
  const seed = transformPortfolioSource();
  return {
    experiences: seed.experiences.map((e, i) => ({ ...e, _id: `exp${i + 1}` })),
    skills: seed.skills.map((s, i) => ({ ...s, _id: `sk${i + 1}` })),
    projects: seed.projects.map((p, i) => ({ ...p, _id: `pr${i + 1}` })),
    socials: seed.socials.map((s, i) => ({ ...s, _id: `soc${i + 1}` })),
